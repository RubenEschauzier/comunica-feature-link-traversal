...
export class ActorOptimizeQueryOperationInitializeSourceLinkTraversalCache extends ActorOptimizeQueryOperation {
  ...
  public async test(action: IActionOptimizeQueryOperation): Promise<TestResult<IActorTest>> {
    ...
    if (!action.context.get(KeysQuerySourceIdentify.traverse)){
      return failTest(`${this.name} only adds the cache as source for traversal queries`);
    }
    if (!cacheManager){
      return failTest(`${this.name} cannot add a cache-based query source without a cache manager object in context`);
    }
    ...
    if (!cacheManager!.getRegisteredCache(CacheEntrySourceState.cacheSourceStateQuerySource)){
      return failTest(`${this.name} requires cache: ${CacheEntrySourceState.cacheSourceStateQuerySource.id}`);
    }
    if (!cacheManager!.getRegisteredView(CacheSourceStateViews.cacheQueryView)){
      return failTest(`${this.name} requires view: ${CacheSourceStateViews.cacheQueryView.id}`);
    }
    return passTestVoid();
  }
