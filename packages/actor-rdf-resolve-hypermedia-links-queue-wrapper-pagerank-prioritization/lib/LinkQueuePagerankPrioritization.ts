 ...
 */
export class LinkQueuePagerankPrioritization extends LinkQueueWrapper<LinkQueuePriority> {
  ...
  public override pop(): ILink | undefined {
    ...
    if (super.getSize() > 0 && this.updated) {
  ...
  public calculatePrioritiesPagerank() {
    ...
    if (this.updated) {
      ...
      const rankings = pagerank(edgeListArray, 0.85, 0.001, (err: any, res: any) => {
        if (err) {
          console.log(err);
        }
      });
