...
import { KEY_CONTEXT_WRAPPED, LinkQueuePriorityBreadthFirst } from '../lib';
import {
  ActorRdfResolveHypermediaLinksQueueWrapperPrioritisationBreadthFirst,
} from '../lib/ActorRdfResolveHypermediaLinksQueueWrapperPrioritisationBreadthFirst';
...
describe('ActorRdfResolveHypermediaLinksQueueWrapperLimitCount', () => {
  ...
  describe('An ActorRdfResolveHypermediaLinksQueueWrapperLimitCount instance', () => {
    let actor: ActorRdfResolveHypermediaLinksQueueWrapperPrioritisationBreadthFirst;
...
>;
    ...
    beforeEach(() => {
      ...
      actor = new ActorRdfResolveHypermediaLinksQueueWrapperPrioritisationBreadthFirst(
    ...
    it('should run', async() => {
      await expect(actor.run({ firstUrl: 'first', context: new ActionContext() })).resolves.toMatchObject({
        linkQueue: new LinkQueuePriorityBreadthFirst(<any> 'inner'),
      });
