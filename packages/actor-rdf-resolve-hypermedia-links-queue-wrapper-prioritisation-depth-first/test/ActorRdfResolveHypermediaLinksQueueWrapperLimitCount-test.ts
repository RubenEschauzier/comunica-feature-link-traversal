...
import { KEY_CONTEXT_WRAPPED, LinkQueuePriorityNonAdaptive } from '..';
import {
  ActorRdfResolveHypermediaLinksQueueWrapperPrioritisationNonAdaptive,
...
describe('ActorRdfResolveHypermediaLinksQueueWrapperLimitCount', () => {
  ...
  describe('An ActorRdfResolveHypermediaLinksQueueWrapperLimitCount instance', () => {
    let actor: ActorRdfResolveHypermediaLinksQueueWrapperPrioritisationNonAdaptive;
    ...
    it('should run', async() => {
      expect(await actor.run({ firstUrl: 'first', context: new ActionContext() })).toMatchObject({
        linkQueue: new LinkQueuePriorityNonAdaptive(<any> 'inner'),
      });
