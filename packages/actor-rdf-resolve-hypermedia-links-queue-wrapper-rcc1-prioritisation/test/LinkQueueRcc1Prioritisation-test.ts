...
import { LinkQueueRcc1Prioritisation } from '..';
...
describe('LinkQueueIndegreePrioritisation', () => {
  ...
  let queue: LinkQueueRcc1Prioritisation;

  let mockedInner: LinkQueuePriority;
  let mockedQueue: LinkQueueRcc1Prioritisation;
  ...
  beforeEach(() => {
    ...
    queue = new LinkQueueRcc1Prioritisation(inner, graph);
    ...
    mockedQueue = new LinkQueueRcc1Prioritisation(mockedInner, graph);
  });
  ...
  describe('Static graph', () => {
    ...
    it('assigns proper priority for URL in graph', () => {
        ...
        expect(queue.pop()).toEqual( {url: 'G', priority: 2, index: 0})
    ...
    it('only calls updatePriority on pop when rcc has changed', () => {
        ...
        queue.pop();
        expect(spy.mock.calls).toEqual([["A", 0], ["B", 3], ["C", 3], ["D", 3], ["E", 3], ["F", 4], ["G", 2]]);
    })
  });
  describe('Changing graph', () => {
    ...
    it('assign correct priority to new link on peek and pop', () => {
        ...
        expect(queue.pop()).toEqual({url: 'D', priority: 7, index: 0});
        expect(queue.peek()).toEqual({url: 'F', priority: 6, index: 0});
    });
    it('assigns correct priority when rcc change between pop and peek', () => {
        ...
        expect(queue.peek()).toEqual({url: 'F', priority: 15, index: 0});
    });
  });
});
