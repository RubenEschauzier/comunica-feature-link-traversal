...
import { LinkQueueRel2Prioritisation } from '..';
...
describe('LinkQueueIndegreePrioritisation', () => {
  ...
  let queue: LinkQueueRel2Prioritisation;

  let mockedInner: LinkQueuePriority;
  let mockedQueue: LinkQueueRel2Prioritisation;
  ...
  beforeEach(() => {
    ...
    queue = new LinkQueueRel2Prioritisation(inner, graph);
    ...
    mockedQueue = new LinkQueueRel2Prioritisation(mockedInner, graph);
  });
  ...
  describe('Static graph', () => {
    ...
    it('only calls updatePriority on pop when rcc has changed', () => {
        ...
        queue.pop();
        expect(spy.mock.calls).toEqual([["F", 3], ["B", 1], ["C", 1], ["D", 1], ["G", 2], ["E", 1]]);
    })
  });
