...
import { LinkQueueRel2Prioritization } from '../lib/LinkQueueRel2Prioritization';

const DF = new DataFactory();
const BF = new BindingsFactory(DF);

describe('LinkQueueRel2Prioritisation', () => {
  let inner: LinkQueuePriority;
  let queue: LinkQueueRel2Prioritization;
  ...
  beforeEach(() => {
    ...
    queue = new LinkQueueRel2Prioritization(inner, statisticTraversalTopologyRcc);
  });
        ...
        try {
          ...
          attributionStream2.on('end', () => {
            try {
              ...
              expect(queue.peek()).toEqual({ url: 'http://C', metadata: { index: 0, priority: 4 }});
              done();
            } catch (error) {
              done(error);
            }
