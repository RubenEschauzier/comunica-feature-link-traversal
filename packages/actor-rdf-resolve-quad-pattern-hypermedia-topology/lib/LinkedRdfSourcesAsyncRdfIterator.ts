...
export abstract class LinkedRdfSourcesAsyncRdfIterator extends BufferedIterator<RDF.Quad> implements RDF.Stream {
  ...
  private mediatorConstructTopology: MediatorConstructTraversedTopology;
  ...
  public constructor(cacheSize: number, subject: RDF.Term, predicate: RDF.Term, object: RDF.Term, graph: RDF.Term,
    firstUrl: string, maxIterators: number, mediatorConstructTopology: MediatorConstructTraversedTopology, options?: BufferedIteratorOptions) {
    ...
    this.mediatorConstructTopology = mediatorConstructTopology;
      ...
      .then(async linkQueue => {
        ...
        while (this.canStartNewIterator() && this.isRunning()) {
          ...
          if (nextLink){
            ...
            await this.mediatorConstructTopology.mediate(action);
          }
          if (nextLink) {
